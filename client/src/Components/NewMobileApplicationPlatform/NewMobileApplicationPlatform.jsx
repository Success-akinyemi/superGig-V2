import './NewMobileApplicationPlatform.css'
import { useEffect, useRef, useState } from 'react'
import { useSelector } from 'react-redux'
import { app } from '../../firebase'
import { getDownloadURL, getStorage, ref, uploadBytesResumable } from 'firebase/storage'
import { newMobileApplicationPlatform } from '../../helper/api'

function NewMobileApplicationPlatform() {
    ...
    const handleNewMobilePlatfrom = async (e) => {
        e.preventDefault()
        try {
            setIsLoading(true)
            console.log(formData)
            const res = await newMobileApplicationPlatform(formData)
        } catch (error) {
            console.log('Unable to create new mobile application platform::', error)
        } finally {
            setIsLoading(false)
        }
    }
  ...
  return (
    <form className='newMobileApplicationPlatform' onSubmit={handleNewMobilePlatfrom}>
        ...
        <div className="imgGroup" onClick={() => fileRef.current.click()} >
              {
                formData.proofImg ? (
                  ...
                  <h5>Upload Music platform icon (Tap to Upload)</h5>
                )
              }
              <input
                type="file"
        ...
        <div className="submitBtn">
            <button onClick={handleNewMobilePlatfrom} disabled={isLoading} >
...
export default NewMobileApplicationPlatform